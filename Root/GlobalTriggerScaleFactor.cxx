...
void
AnaQ::GlobalTriggerScaleFactor::initialize(unsigned int slot,
                                           unsigned long long,
                                           unsigned long long)
{
  for (auto const& elecTrig : m_elecTriggers) {
    ...
    m_elecEffTools_handles.emplace_back(
      ...
      elecTrig + "_" + m_sysSet.name() + "_" + std::to_string(slot)+"_"+std::to_string(m_index));
  ...
  m_trigGlobalTool_handle =
    asg::AnaToolHandle<ITrigGlobalEfficiencyCorrectionTool>(
      "TrigGlobalEfficiencyCorrectionTool/TrigGlobalEfficiencyCorrection_"+std::to_string(slot)+"_"+std::to_string(s_nInstances));
  // HARDCODED FOR NOW
...
double
AnaQ::GlobalTriggerScaleFactor::evaluate(
  Observable<ConstDataVector<xAOD::ElectronContainer>> electrons) const
{
  ...
  for (auto elItr : *(electrons)) {
    for (auto& elecEffTool_handle : m_elecEffTools_handles) {
      double effSF = -1.0;
      auto status =
        elecEffTool_handle->getEfficiencyScaleFactor(*elItr, effSF);
    ...
    for (auto& elecSFTool_handle : m_elecSFTools_handles) {
      double effSF = -1.0;
      auto status =
        elecSFTool_handle->getEfficiencyScaleFactor(*elItr, effSF);
      if (status == CP::CorrectionCode::Error) {
        throw std::runtime_error(
          "problem in electron efficiency correction tool");
      } else if (status == CP::CorrectionCode::OutOfValidityRange) {
  ...
  auto status = m_trigGlobalTool_handle->getEfficiencyScaleFactor(triggeringElectrons, globTrigSF);
  return globTrigSF;
}

void AnaQ::GlobalTriggerScaleFactor::finalize(unsigned int) {
  m_elecEffTools_handles.clear();
  m_elecEffTools_handleArray.clear();
  m_elecSFTools_handles.clear();
  m_elecSFTools_handleArray.clear();
}

bool
AnaQ::GlobalTriggerScaleFactor::applySystematicVariation(
