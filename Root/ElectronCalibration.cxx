...
AnaQ::ElectronCalibration::ElectronCalibration(ElectronCalibration::Config cfg)
    : m_cfg(cfg) {}

void AnaQ::ElectronCalibration::initialize(unsigned int, unsigned long long,
                                           unsigned long long) {
  m_outAuxContainerName =
      m_cfg.outContainerName + "Aux."; // the period is very important!
  m_outSCContainerName = m_cfg.outContainerName + "ShallowCopy";
  m_outSCAuxContainerName =
      m_outSCContainerName + "Aux."; // the period is very important!
  ...
  m_EgammaCalibrationAndSmearingTool->setProperty("decorrelationModel",
                                                  m_cfg.decorrelationModel);
  if (m_cfg.useFastSim) {
    // ANA_CHECK( m_EgammaCalibrationAndSmearingTool->setProperty("useFastSim",
    // 1));
  ...
  m_systList = EventHelpers::getListofSystematics(recSyst, m_cfg.systMode,
                                                  m_cfg.systVal);
...
AnaQ::ElectronCalibration::evaluate(
  ...
  for (const auto &syst_it : m_systList) {
    ...
    if (m_EgammaCalibrationAndSmearingTool->applySystematicVariation(syst_it) !=
      ...
      throw std::runtime_error(
          "Failed to configure EgammaCalibrationAndSmearingTool for "
          "systematic ");
      // ANA_MSG_ERROR( "Failed to configure EgammaCalibrationAndSmearingTool
    ...
    auto calibEls =
        EventHelpers::makeConstDataVector(shallowCopy.elements.get());
