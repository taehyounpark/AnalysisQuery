...
void AnaQ::PileupReweight::initialize(unsigned int slot, unsigned long long, unsigned long long) {
  // resolve paths
...
float AnaQ::PileupReweight::evaluate(AnaQ::Observable<xAOD::EventInfo> eventInfo) const {
  if ( m_prwTool_handle->applySystematicVariation(m_sysSet) != EL::StatusCode::SUCCESS ) {
    throw std::runtime_error("Failed to apply systematic variation '"+m_sysSet.name()+"' on pileup reweighting tool");
  }
